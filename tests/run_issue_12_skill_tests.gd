...
func _test_skill_progress_rules() -> void:
	...
	var points_before_refund := progress.unspent_points
	var decrease_result := progress.decrease_rank(&"blade_wave", &"traveler")
	_expect(decrease_result.ok and int(decrease_result.rank) == 4, "已学习技能可以降低一级")
	_expect(progress.unspent_points == points_before_refund + 1, "技能降级返还一点技能点")
	_expect(not progress.decrease_rank(&"star_bolt", &"traveler").ok, "不能调整其他职业技能")
...
func _test_skill_window_ui() -> void:
	...
	var echoed_open := _key_event(KEY_K, true)
	level._unhandled_input(echoed_open)
	_expect(not level._skills_panel.visible, "键盘 echo 不会重复打开技能窗口")
	level._unhandled_input(_key_event(KEY_K))
	_expect(level._skills_panel.visible, "K 键打开职业技能窗口")
	...
	level._unhandled_input(_key_event(KEY_Z))
	_expect(player.get_skill_rank(&"blade_wave") == 0, "技能窗口不再使用 Z 键升级技能")
	level._skills_increase_button.pressed.emit()
	...
	_expect(player.get_skill_quickbar()[0] == &"", "技能降到零级时清理对应快捷栏")
