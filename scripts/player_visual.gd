...
func _draw_sword_attack() -> void:
	...
	var length := 58.0 if heavy else 42.0
	...
	draw_line(hand, tip, Color("d9edf4"), 7.0 if heavy else 5.0)
	draw_line(hand, tip, Color("6f8792"), 2.0)
	draw_circle(tip, 6.0 if heavy else 4.0, Color("f0b85f") if heavy else Color("c9eef5"))
...
func _draw_staff_attack() -> void:
	...
	draw_line(start, tip, Color("76553c"), 6.0)
	draw_circle(tip, 12.0 if heavy else 8.0, Color("9f6df4") if heavy else Color("58c9ef"))
	draw_circle(tip, 5.0 if heavy else 3.0, Color(1.0, 1.0, 1.0, 0.9))
